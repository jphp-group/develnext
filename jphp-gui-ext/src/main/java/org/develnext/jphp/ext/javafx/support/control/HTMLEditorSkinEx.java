...
public class HTMLEditorSkinEx extends BehaviorSkinBase<HTMLEditorEx, HTMLEditorBehaviorEx> {
...
//    private Button redoButton;
    ...
    public HTMLEditorSkinEx(HTMLEditorEx htmlEditor) {
        super(htmlEditor, new HTMLEditorBehaviorEx(htmlEditor));
...
//        populateToolbars();
        ...
        webView.addEventHandler(MouseEvent.MOUSE_RELEASED, new EventHandler<MouseEvent>() {
            @Override
        ...
        webView.addEventHandler(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
            ...
            public void handle(final KeyEvent event) {
                HTMLEditorSkinEx.this.applyTextFormatting();
                ...
                if (event.getCode() == KeyCode.TAB && !event.isControlDown()) {
                    ...
                    */
                        if (HTMLEditorSkinEx.this.getCommandState(BULLETS_COMMAND) || HTMLEditorSkinEx.this.getCommandState(NUMBERS_COMMAND)) {
                            HTMLEditorSkinEx.this.executeCommand(INDENT_COMMAND, null);
                        } else {
                            HTMLEditorSkinEx.this.executeCommand(INSERT_TAB_COMMAND, null);
                        }
                    } else {
                    /*
                    ** if we are in either Bullet or Numbers mode then the
                    ...
                    */
                        if (HTMLEditorSkinEx.this.getCommandState(BULLETS_COMMAND) || HTMLEditorSkinEx.this.getCommandState(NUMBERS_COMMAND)) {
                            HTMLEditorSkinEx.this.executeCommand(OUTDENT_COMMAND, null);
                        }
                    }
                    return;
                }
                    ...
                    public void run() {
                        if (webPage.getClientSelectedText().isEmpty()) {
                            if (event.getCode() == KeyCode.UP || event.getCode() == KeyCode.DOWN ||
                                ...
                                HTMLEditorSkinEx.this.updateToolbarState(true);
                            } else if (event.isControlDown() || event.isMetaDown()) {
                                if (event.getCode() == KeyCode.B) {
                                    HTMLEditorSkinEx.this.keyboardShortcuts(BOLD_COMMAND);
                                } else if (event.getCode() == KeyCode.I) {
                                    HTMLEditorSkinEx.this.keyboardShortcuts(ITALIC_COMMAND);
                                } else if (event.getCode() == KeyCode.U) {
                                    HTMLEditorSkinEx.this.keyboardShortcuts(UNDERLINE_COMMAND);
                                }
                                HTMLEditorSkinEx.this.updateToolbarState(true);
                            } else {
                                resetToolbarState = event.getCode() == KeyCode.ENTER;
                                if (resetToolbarState) {
                                    if (HTMLEditorSkinEx.this.getCommandState(BOLD_COMMAND) != boldButton.selectedProperty().getValue()) {
                                        HTMLEditorSkinEx.this.executeCommand(BOLD_COMMAND, boldButton.selectedProperty().getValue().toString());
                                    }
                                }
                                HTMLEditorSkinEx.this.updateToolbarState(false);
                            }
...
//                }
                ...
                HTMLEditorSkinEx.this.pseudoClassStateChanged(CONTAINS_FOCUS_PSEUDOCLASS_STATE, newValue);
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                ...
                if (newValue.doubleValue() == totalWork) {
                    ...
                    Platform.runLater(new Runnable() {
                        @Override
                        public void run() {
                            HTMLEditorSkinEx.this.setContentEditable(true);
                            HTMLEditorSkinEx.this.updateToolbarState(true);
                            HTMLEditorSkinEx.this.updateNodeOrientation();
                        }
                    });
                }
            }
...
//        });
        ...
        formatComboBox.valueProperty().addListener(new ChangeListener<String>() {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                if (newValue == null) {
                    ...
                    HTMLEditorSkinEx.this.executeCommand(FORMAT_COMMAND, formatValue);
                    HTMLEditorSkinEx.this.updateToolbarState(false);
                    ...
                    for (int i = 0; i < DEFAULT_FORMAT_MAPPINGS.length; i++) {
                        ...
                        if (mapping[0].equalsIgnoreCase(formatValue)) {
                            HTMLEditorSkinEx.this.executeCommand(FONT_SIZE_COMMAND, mapping[2]);
                            HTMLEditorSkinEx.this.updateToolbarState(false);
                            break;
                        }
                    }
        ...
        fontFamilyComboBox.valueProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                HTMLEditorSkinEx.this.executeCommand(FONT_FAMILY_COMMAND, newValue);
            }
        });
        ...
        fontSizeComboBox.valueProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                Object fontSizeValue = HTMLEditorSkinEx.this.getCommandValue(FONT_SIZE_COMMAND);
                if (!newValue.equals(fontSizeValue)) {
                    HTMLEditorSkinEx.this.executeCommand(FONT_SIZE_COMMAND, fontSizeMap.get(newValue));
                }
            }
        });
        ...
        insertHorizontalRuleButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                HTMLEditorSkinEx.this.executeCommand(INSERT_NEW_LINE_COMMAND, null);
                HTMLEditorSkinEx.this.executeCommand(INSERT_HORIZONTAL_RULE_COMMAND, null);
                HTMLEditorSkinEx.this.updateToolbarState(false);
            }
        });
        ...
        String fgIcon = AccessController.doPrivileged((PrivilegedAction<String>) new PrivilegedAction<String>() {
            @Override
            public String run() {
        ...
        fgColorButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent ev1) {
                ...
                if (newValue != null) {
                    HTMLEditorSkinEx.this.executeCommand(FOREGROUND_COLOR_COMMAND, colorValueToHex(newValue));
                    fgColorButton.hide();
                }
            }
        });
        ...
        String bgIcon = AccessController.doPrivileged((PrivilegedAction<String>) new PrivilegedAction<String>() {
            @Override
            public String run() {
        ...
        bgColorButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent ev) {
                ...
                if (newValue != null) {
                    HTMLEditorSkinEx.this.executeCommand(BACKGROUND_COLOR_COMMAND, colorValueToHex(newValue));
                    bgColorButton.hide();
                }
            }
        });
    ...
    private Button addButton(ToolBar toolbar, final String iconName, String tooltipText,
        ...
        Image icon = AccessController.doPrivileged((PrivilegedAction<Image>) new PrivilegedAction<Image>() {
            @Override
            public Image run() {
...
//        button.setGraphic(new ImageView(icon));
        ...
        button.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                HTMLEditorSkinEx.this.executeCommand(command, null);
                HTMLEditorSkinEx.this.updateToolbarState(false);
            }
        });
...
//        toggleButton.setGraphic(new ImageView(icon));
        ...
        if (!BOLD_COMMAND.equals(command)) {
            toggleButton.selectedProperty().addListener(new ChangeListener<Boolean>() {
                @Override
                public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                    if (HTMLEditorSkinEx.this.getCommandState(command) != newValue.booleanValue()) {
                        HTMLEditorSkinEx.this.executeCommand(command, null);
                    }
                }
            });
...
//        redoButton.setDisable(!isCommandEnabled(FORMAT_COMMAND));
    ...
    private void enableToolbar(final boolean enable) {
        Platform.runLater(new Runnable() {
            ...
            */
                if (enable) {
                    copyButton.setDisable(!HTMLEditorSkinEx.this.isCommandEnabled(COPY_COMMAND));
                    cutButton.setDisable(!HTMLEditorSkinEx.this.isCommandEnabled(CUT_COMMAND));
                    pasteButton.setDisable(!HTMLEditorSkinEx.this.isCommandEnabled(PASTE_COMMAND));
                } else {
