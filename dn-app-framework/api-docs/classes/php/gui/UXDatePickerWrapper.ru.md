# UXDatePickerWrapper

- **класс** `UXDatePickerWrapper` (`php\gui\UXDatePickerWrapper`) **унаследован от** [`UXNodeWrapper`](https://github.com/jphp-compiler/develnext/blob/master/dn-app-framework/api-docs/classes/php/gui/UXNodeWrapper.ru.md)
- **пакет** `framework`
- **исходники** `php/gui/UXDatePickerWrapper.php`

**Описание**

Class UXDatePickerWrapper

---

#### Методы

- `->`[`applyData()`](#method-applydata)
- См. также в родительском классе [UXNodeWrapper](https://github.com/jphp-compiler/develnext/blob/master/dn-app-framework/api-docs/classes/php/gui/UXNodeWrapper.ru.md)

---
# Методы

<a name="method-applydata"></a>

### applyData()
```php
applyData(php\gui\UXData $data): void
```